...
class Pathfinder(Node):
  ...
  def __init__(self):
    super().__init__("go_to_loc")
    self.coeff = 1.5
    self.pose_thershold_linear = 1
    self.pose_thershold_angular = 0.01
    ...
    self.new_turtle_subscriber_ = self.create_subscription(TurtleArray, "new_turtles",
                                                           self.callback_new_turtles, 10)
    
    self.timer = self.create_timer(0.5, self.controller)

    self.get_logger().info("Pathfinder has started.")
  ...
  def controller(self):
    ...
    if abs(target_theta - self.pose_.theta) > self.pose_thershold_angular:
      msg.angular.z = (target_theta - self.pose_.theta) * self.coeff
    
    else:
      if distance >= self.pose_thershold_linear:
        ...
        self.get_logger().info("Turtle has reached the set point")
  ...
  def callback_call_catch_turtle(self, future, turtle_name):
    try:
      ...
      if respons.success:
          self.get_logger().info(f"Turtle: {turtle_name} was caught.")
      else:
        self.get_logger().info("Turtle could not be caught.")
